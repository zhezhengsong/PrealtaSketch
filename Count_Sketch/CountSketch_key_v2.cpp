...
int main(int argc, const char* argv[]) {
    ...
    // Generate a random key (not from CLI)
    uint64_t key = make_random_key();
    cout << "CountSketch key = 0x" << hex << key << dec << "\n";

    if (argc < 4) {
        cerr << "Usage: " << argv[0] << " <s> <k> <clustering_resolution>\n";
        cerr << "Example: " << argv[0] << " 1000 20 1.20\n";
        return 1;
    }
    int s = stoi(argv[1]);
    int k = stoi(argv[2]);
    double clustering_resolution = stod(argv[3]);
    ...
    // Clustering (with configurable resolution)
